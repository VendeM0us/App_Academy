...
def factors(num)
  ...
  (1..num).each do |possible_factors|
    factors << possible_factors if num % possible_factors == 0
  end
...
class Array
  ...
  def bubble_sort(&prc)
    ...
    while !sorted
      ...
      (0...sort.length - 1).each do |i1|
        i2 = i1 + 1
        if prc.call(sort[i1], sort[i2]) == 1
          sort[i1], sort[i2] = sort[i2], sort[i1]
          sorted = false
        end
      end
    end
...
def substrings(string)
  ...
  (0...string.length).each do |current_index|
    (current_index...string.length).each do |last_index|
      substring = string[current_index..last_index]
...
def subwords(word, dictionary)
  ...
  word_substrings.each { |substring| subwords_array << substring if dictionary.include?(substring) }
  subwords_array
end
...
def concatenate(strings)
  strings.my_inject { |sentence, word| sentence + word }
end